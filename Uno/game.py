...
parser.add_argument('-decksize',type=int, dest='decksize',help=' The size of the deck in question')
parser.add_argument('-handsize',type=int, dest='handsize',help=' The size of the intial hand in question')
...
class Game():
    # Note the constructor arguements, you need to specify the direction of the game 1 is clockwise, intialhandnum is how many cards everyone starts with, and deck_size is obvious
    def __init__(self,numPlayers,direction,intialhandnum,deck_size):
        if intialhandnum*numPlayers > deck_size:
            raise ValueError("Invalid Intial Configuration Chosen, not enough cards")
        self.current_player = 0
        # generate the cards for each player
        self.deck = self.generateDeck(deck_size)
        self.players = [Player(name="Player {}".format(i)) for i in range(numPlayers)]
        ...
        self.direction = 1
        # this function populates our deck with a weighted probability distribution for the cards
    ...
    def start_game(self):
        ...
        self.add_card(self.players[self.current_player].cards[-1],self.players[self.current_player])
        # In case the computer starts the Uno Game, we pick blue to start with if its a TBD card:
    ...
    def evaluate_card(self,card):
        if card.type == "changeColor" or card.type == "add4":
            color = input("Please specify what color you are changing the board to")
            if self.isValid(color):
                if card.type == "add4":
                    for i in range(4):
                        self.players[(self.current_player+self.direction) % len(self.players)].add_card(self.draw())
    ...
    def needs_draw(self):
        return self.players[self.current_player].colors.get(self.cards[-1].color,0) == 0 and self.players[self.current_player].colors.get("TBD",0) == 0
    ...
    def print_hands(self,currentPlayer):
        for player in self.players:
            ...
            if len(player.cards) == 1:
                #TODO replace this with a weighted random number generator based on how long the user took to input this
    ...
    def print_my_hand(self,currentPlayer):
        ...
        for index, card in enumerate(currentPlayer.cards):
            print(card," ",index)
        print("You({}) need to play a card of color {}".format(currentPlayer.name,self.cards[-1].color))
...
def main():
    ...
    while not myGame.game_over():
        ...
        try :
            card_index = int(input("What card index do you pick, please note INDEX"))
            while not myGame.add_card(currentPlayer.cards[card_index],currentPlayer):
                print("YOU PICKED A CARD THAT WAS NOT THE SAME COLOR AS THE PREVIOUS CARD PICK ANOTHER ONE")
                card_index = int(input("What card index do you pick, please note INDEX"))
        except:
            raise ValueError("Please only provide indices as an input for picking a card")
